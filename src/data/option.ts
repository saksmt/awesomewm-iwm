...
export abstract class Option<T> {
  ...
  abstract filter(p: (v: T) => boolean): Option<T>;
  filterNot(p: (v: T) => boolean): Option<T> {
    return this.filter((it) => !p(it));
  }
...
class Some<T> extends Option<T> {
  ...
  filter(p: (v: T) => boolean): Option<T> {
    return p(this.value) ? this : _none;
  }
...
class None<T> extends Option<T> {
  ...
  filter: (_: (v: T) => boolean) => Option<T> = (_) => _none;
  unwrap: () => T | null = () => null;
